...
class TestsView(LoginRequiredMixin, View):
    ...
    def get_performance_tests_forms(self, obj=None):
        ...
        dataset = [model_to_dict(
            x, fields=['test_type', 'routine', 'type', 'special']) for x in dataset]
        formset = PerformanceTestFormSet(
            initial=dataset,
    ...
    def get(self, request, *args, **kwargs):
        ...
        ElectricResistanceTest.objects.get_or_create(
            induction_motor=inductionMotorReport)
        TemperatureRiseTest.objects.get_or_create(
            induction_motor=inductionMotorReport)
        PerformanceDeterminationTest.objects.get_or_create(
            induction_motor=inductionMotorReport)
        NoLoadTest.objects.get_or_create(induction_motor=inductionMotorReport)
        WithstandVoltageACTest.objects.get_or_create(
            induction_motor=inductionMotorReport)
        InsulationResistanceTest.objects.get_or_create(
            induction_motor=inductionMotorReport)
        LockRotorTest.objects.get_or_create(
            induction_motor=inductionMotorReport)
        if not inductionMotorReport:
            ...
            temperature_rise_form = TemperatureRiseTestForm(
                instance=inductionMotorReport.temperature_rise_test)
            ...
            no_load_form = NoLoadTestForm(
                instance=inductionMotorReport.no_load_test)
            withstand_voltage_form = WithstandVoltageACTestForm(
                instance=inductionMotorReport.withstand_voltage_ac_test)
            ...
            context['edit_form'] = InitialForm(
                initial=model_to_dict(inductionMotorReport))
            context['edit_formset'] = self.get_performance_tests_forms(
                inductionMotorReport)
            context['status'] = get_form_statuses(inductionMotorReport)
            context['all_test_completed'] = all(
                value == 'COMPLETED' for value in context['status'].values())
...
class ReportView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        selected_tests = PerformanceTest.objects.filter(
            motor_id=induction_motor.id, status=PerformanceTest.COMPLETED)
...
class GeneratePDF(PDFView):
    def get(self, request, *args, **kwargs):
        ...
        report_url = request.build_absolute_uri(
            reverse('report', kwargs={'id': kwargs['id']}))
        ...
        if pdf:
            InductionMotor.objects.filter(
                id=kwargs['id']).update(report_link=report_link)
...
class DeleteReportView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            InductionMotor.objects.filter(pk=kwargs['id']).update(
                status=InductionMotor.DELETE)
...
class ElectricFormSaveView(View):
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(motor, 'electric_resistance_test'):
            motor.electric_resistance_test = ElectricResistanceTest.objects.get_or_create(
                induction_motor=motor)

        default = Decimal('0.00')
        resistance_ohm_1 = request.POST.get(
            'electric_resistance_test-resistance_ohm_1')
        resistance_ohm_2 = request.POST.get(
            'electric_resistance_test-resistance_ohm_2')
        resistance_ohm_3 = request.POST.get(
            'electric_resistance_test-resistance_ohm_3')
        ambient_temperature_C = request.POST.get(
            'electric_resistance_test-ambient_temperature_C')
        unbalance_percentage = request.POST.get(
...
class TemperatureFormSaveView(View):
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(motor, 'temperature_rise_test'):
            motor.temperature_rise_test = TemperatureRiseTest.objects.get_or_create(
                induction_motor=motor)
...
class NoLoadFormSaveView(View):
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(motor, 'no_load_test'):
            motor.no_load_test = NoLoadTest.objects.get_or_create(
                induction_motor=motor)
        ...
        direction_of_rotation = request.POST.get(
            'no_load_test-direction_of_rotation')
        ...
        PerformanceTest.objects.filter(motor=motor, test_type='no_load_test').update(
            status=PerformanceTest.COMPLETED)
...
class WithStandVoltageFormSaveView(View):
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(motor, 'withstand_voltage_ac_test'):
            motor.withstand_voltage_ac_test = WithstandVoltageACTest.objects.get_or_create(
                induction_motor=motor)
        ...
        time_in_seconds = request.POST.get(
            'withstand_voltage_ac_test-time_in_seconds')
        motor.withstand_voltage_ac_test.description = description if description else ''
        motor.withstand_voltage_ac_test.voltage_kv = voltage_kv if voltage_kv else Decimal(
            '0.00')
...
class InsulationFormSaveView(View):
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(motor, 'insulation_resistance_test'):
            motor.insulation_resistance_test = InsulationResistanceTest.objects.get_or_create(
                induction_motor=motor)

        default = Decimal('0.00')
        description = request.POST.get(
            'insulation_resistance_test-description')
        voltage = request.POST.get('insulation_resistance_test-voltage')
        insulation_resistance = request.POST.get(
        ...
        humidity_percentage = request.POST.get(
...
class LockRotorFormSave(View):
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(motor, 'lock_rotor_test'):
            motor.lock_rotor_test = LockRotorTest.objects.create(
                induction_motor=motor)
...
class PerformanceDeterminationFormSave(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not hasattr(motor, 'performance_determination_test'):
            motor.performance_determination_test = PerformanceDeterminationTest(
                induction_motor=motor)
        ...
        frequency = request.POST.get(
            'performance_determination_test-frequency')
        nominal_t = request.POST.get(
            'performance_determination_test-nominal_t')
